
fila_uno   = ["0","0",".","0","0"]
fila_dos   = ["0",".","0",".","0"]
fila_tres  = [".","0","0","0","."]
fila_cuatro= ["0",".","0",".","0"]
fila_cinco = ["0","0",".","0","0"]

primerNivel=[fila_uno,fila_dos,fila_tres,fila_cuatro,fila_cinco]

nivelDosfila_uno    =[".","0",".","0","."]
nivelDosfila_dos    =["0","0",".","0","0"]
nivelDosfila_tres   =[".","0",".","0","."]
nivelDosfila_cuatro =["0",".","0",".","0"]
nivelDosfila_cinco  =["0",".","0",".","0"]

segundoNivel=[nivelDosfila_uno,nivelDosfila_dos,nivelDosfila_tres,nivelDosfila_cuatro,nivelDosfila_cinco]

nivelTresfila_uno    =["0",".",".",".","0"]
nivelTresfila_dos    =["0","0",".","0","0"]
nivelTresfila_tres   =[".",".","0",".","."]
nivelTresfila_cuatro =["0",".","0",".","."]
nivelTresfila_cinco  =["0",".","0","0","."]

tercerNivel=[nivelTresfila_uno,nivelTresfila_dos,nivelTresfila_tres,nivelTresfila_cuatro,nivelTresfila_cinco]

nivelCuatrofila_uno    =["0","0",".","0","0"]
nivelCuatrofila_dos    =[".",".",".",".","."]
nivelCuatrofila_tres   =["0","0",".","0","0"]
nivelCuatrofila_cuatro =[".",".",".",".","0"]
nivelCuatrofila_cinco  =["0","0",".",".","."]

cuartoNivel=[nivelCuatrofila_uno,nivelCuatrofila_dos,nivelCuatrofila_tres,nivelCuatrofila_cuatro,nivelCuatrofila_cinco]

nivelCincofila_uno    =[".",".",".","0","0"]
nivelCincofila_dos    =[".",".",".","0","0"]
nivelCincofila_tres   =[".",".",".",".","."]
nivelCincofila_cuatro =["0","0",".",".","."]
nivelCincofila_cinco  =["0","0",".",".","."]

quintoNivel=[nivelCincofila_uno,nivelCincofila_dos,nivelCincofila_tres,nivelCincofila_cuatro,nivelCincofila_cinco]

def getTablero(nivel):
    if nivel == 1:
        return primerNivel

    if nivel == 2:
        return segundoNivel

    if nivel == 3:
        return  tercerNivel

    if nivel == 4:
        return cuartoNivel

    if nivel == 5:
        return quintoNivel


